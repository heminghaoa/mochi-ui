...
Pokoland UI 的所有视觉决策都收敛在这份文档里。新组件必须从这些令牌取值,不允许硬编码颜色或阴影。
...
- sprite 权威来源 `src/icons.svg`(25 个 symbol),demo 内联副本由 `bun run build` 同步(标记区勿手改)
